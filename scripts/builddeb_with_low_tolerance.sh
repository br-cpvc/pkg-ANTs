...
if [[ "$old_method" == true ]]; then
    ...
    source ${script_dir}/patch_itk_tolerance.sh $outputdir/build/ITKv5
    cd $outputdir/build
...
cp $bindir/Atropos ${deb_root}/usr/bin/Atropos_wlt
cp $bindir/ImageMath ${deb_root}/usr/bin/ImageMath_wlt
cp $bindir/N3BiasFieldCorrection ${deb_root}/usr/bin/N3BiasFieldCorrection_wlt
cp $bindir/N4BiasFieldCorrection ${deb_root}/usr/bin/N4BiasFieldCorrection_wlt
